...
def configure_yamaha_for_measurement():
    ...
    try:
        # Configure ALSA card 1 for Asus Zephyrus G16 studio mic array
        subprocess.run(["amixer", "-c", "1", "cset", "numid=13", "0"], capture_output=True)
        subprocess.run(["amixer", "-c", "1", "cset", "numid=16", "0"], capture_output=True)
        subprocess.run(["amixer", "-c", "1", "cset", "numid=14", "48"], capture_output=True)
    except Exception:
        pass
    ...
    try:
        ...
        if v is not None and v.text not in ["0", None, ""]:
    ...
    ync_cmd('<YAMAHA_AV cmd="PUT"><System><Speaker_Preout><Pattern_1><PEQ><Sel>Through</Sel></PEQ></Pattern_1></Speaker_Preout></System></YAMAHA_AV>')
    ...
    res_peq = ync_cmd('<YAMAHA_AV cmd="GET"><System><Speaker_Preout><Pattern_1><PEQ><Sel>GetParam</Sel></PEQ></Pattern_1></Speaker_Preout></System></YAMAHA_AV>')
    if '<Sel>Through</Sel>' not in res_peq:
...
def measure_channel(wav_path, ch_name):
    ...
    if len(data_rec.shape) > 1 and data_rec.shape[1] > 1:
        rms_0 = np.sqrt(np.mean(data_rec[:, 0].astype(float)**2))
        rms_1 = np.sqrt(np.mean(data_rec[:, 1].astype(float)**2))
        active_ch = 1 if rms_1 >= rms_0 else 0
        raw_samples = data_rec[:, active_ch]
    else:
        raw_samples = data_rec.flatten()
