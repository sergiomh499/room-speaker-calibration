...
- [ ] T001 Initialize export artifact destination directory structure in `exports/`
- [ ] T002 Create test harness and unit test suite in `tests/test_filter_export.py`
...
- [ ] T003 Implement `format_rew()` function producing standard Room EQ Wizard `.req` text for single channels in `scripts/export_filters.py`
- [ ] T004 Implement `format_equalizer_apo()` function producing dual-channel config with automatic negative preamp calculation in `scripts/export_filters.py`
- [ ] T005 Implement `format_csv()` function producing structured tabular CSV output with active hardware tags in `scripts/export_filters.py`
- [ ] T006 Implement `build_export_bundle()` packaging all formats into in-memory dictionary and ZIP archive in `scripts/export_filters.py`
...
- [ ] T007 [US1] Wire active hardware profile (`config/hardware.json`) and target profile ingestion into `scripts/export_filters.py`
- [ ] T008 [US1] Add automated unit tests for REW and EqualizerAPO formatting edge cases (inactive 0 dB bands, extreme Q factors) in `tests/test_filter_export.py`
...
- [ ] T009 [US2] Implement `GET /api/export_filters` endpoint supporting `format=rew`, `equalizerapo`, `csv`, and `all` in `scripts/web_calibration_server.py`
- [ ] T010 [US2] Implement `#export-filters-card` HTML markup and CSS buttons in `scripts/web_calibration_server.py`
- [ ] T011 [US2] Implement client-side JavaScript handlers to dynamically update export download URLs when the active profile changes in `scripts/web_calibration_server.py`
...
- [ ] T012 [US3] Implement CLI `argparse` entry point with `--profile`, `--format`, and `--out-dir` options in `scripts/export_filters.py`
- [ ] T013 [US3] Add automated CLI execution test in `tests/test_filter_export.py`
...
- [ ] T014 Execute full project test suite via `python3 -m unittest discover -s tests -p "test_*.py"`
- [ ] T015 Restart live calibration background service `cal_server` on port 53317 and verify export endpoints
- [ ] T016 Verify Constitution Principles I–V (confirm export endpoints perform zero destructive writes on the AVR)
...
- **User Story 2 (P2)** depends on US1 (requires format generators).
- **User Story 3 (P3)** depends on US1 and Phase 2.
