...
| **Modal Diagnostic in Step 3** | Replaces early hardware preflight placement with post-measurement evaluation in Step 3 (Optimization & Profiles) to contextualize room physics against calculated PEQ. | Placing diagnostic in Step 1 before acoustic data has been captured. |
| **SVG Vector Visualizations** | Vector graphics scale cleanly on mobile viewports and eliminate fragile text wrapping inherent to ASCII art diagrams. | Monospace ASCII diagrams. |
| **Client-side Async Error Guards** | Comprehensive `try/catch/finally` blocks protect the client state machine from unhandled promise rejections that break wizard navigation. | Unguarded top-level asynchronous event handlers. |
